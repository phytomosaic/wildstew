...
load('e.rda', verbose=T)  # data in same directory as app.R
...
ui <- fluidPage(
        ...
        sidebarLayout(
                ...
                mainPanel(
                        ...
                        hr(),
                        div(class='footer',
                            p('Creating PDF may take a few seconds, please be patient.'),
                            # p('Caution! This app is in beta stage for trial only!'),
                            # p('Data are current as of 2021.'),
...
server <- function(input, output) {
        ...
        output$downloadreport <- downloadHandler(
                ...
                content = function(file) {
                        src       <- normalizePath('report_TRYELEMENTAL.Rmd')
                        src_png_a <- normalizePath('usfs.png')
                        src_png_b <- normalizePath('bothlichens.png')
                        owd <- setwd(tempdir())
                        on.exit(setwd(owd))
                        file.copy(src, 'report_TRYELEMENTAL.Rmd',
                                  overwrite=TRUE)
