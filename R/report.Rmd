...
Based on **critical loads exceedances**, this Wilderness ranks in the *xx*^th^ percentile nationally for N deposition and *xx*^th^ percentile for S deposition.^[See **[HERE](#further-information)** for methods.]
...
```{r aq-trends}
### TODO
# aq_trend   <- c('IMPROVING','STABLE','DETERIORATING')[now > then]
# ndep_trend <- c('DECREASING','STABLE','INCREASING')[now > then]
# sdep_trend <- c('DECREASING','STABLE','INCREASING')[now > then]
aq_trend   <- 'IMPROVING'
ndep_trend <- 'DECREASING'
sdep_trend <- 'DECREASING'
```

Over the past ten years (relative to the preceding decade):  

\LARGE
Air quality is apparently **`r aq_trend`**.

N deposition is apparently **`r ndep_trend`**.

S deposition is apparently **`r sdep_trend`**.
\normalsize

## N trends plot
```{r plot-n-trends, fig.height=3, fig.cap='Nitrogen trends.', warning=FALSE}
ecole::set_par(3)
plot_trend(pick = rx(), yvar = 'n_airscore',
           ylab = expression(N~air~score~(kg~N~ha^'-1'~y^'-1')))
plot_trend(pick = rx(), yvar = 'n_ex_sr_sens',
           ylab = 'N CL exceedance,\nsensitive species')
plot_trend(pick = rx(), yvar = 'n_ex_compn',
           ylab = 'N CL exceedance,\ncomm composition')
```

## S trends plot
```{r plot-s-trends, fig.height=3, fig.cap='Sulfur trends.', warning=FALSE}
ecole::set_par(3)
plot_trend(pick = rx(), yvar = 's_airscore',
           ylab = expression(S~air~score~(kg~S~ha^'-1'~y^'-1')))
plot_trend(pick = rx(), yvar = 's_ex_sr_sens',
           ylab = 'S CL exceedance,\nsensitive species')
plot_trend(pick = rx(), yvar = 's_ex_compn',
           ylab = 'S CL exceedance,\ncomm composition')
```
...
WAQVs are supported by monitoring changes in “sensitive receptors” like epiphytic macrolichens.  Lichens are impacted by air pollutants and therefore indicate changing conditions.  Workers long ago established links between lichen tissue elemental content and the atmospheric deposition of pollutants such as oxidized nitrogen (NOx), reduced nitrogen (NHx) and oxidized sulfur (SOx).  Each lichen species also has a unique and individualistic tolerance to pollutants, so observing collective responses is far more robust than any single species alone.
...
Number of species observed: `r x$rich`.  
...
List of all macrolichens observed in plots at `r input$x` over the years `r rng[1]` to `r rng[2]`.  True diversity is likely greater because plot footprints are just a fraction of total Wilderness area.  'Fxl grp' = functional group, where 'cyano' = cyanolichens capable of nitrogen-fixation, and 'forage' = forage lichens important for wildlife.  'N sensitivity' for each species ranges from low to high tolerance (oligo-, meso-, eutroph), as does 'S sensitivity' (sensitive, intermediate, tolerant).  'CL' = critical load resulting in 20\% decrease in occurrence.  'Conservation' = conservation status from NatureServe.
