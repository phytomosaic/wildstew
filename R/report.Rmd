...
```{r plot-plot_valuebar_N, fig.height=2.25, fig.width=6.0, fig.cap='Observed N deposition from CMAQ model (arrow) for the most recent 10-year sample period, in relation to levels of concern.  Levels of concern (Acceptable, Marginal, Concerning, Poor, Very poor) are defined by the expected decline of sensitive lichens (0, 0-20, 20-50, 50-80, and >80 percent declines established for critical loads).'}
...
`plot_valuebar` <- function(pick, brk=b, select='n', ...){
  ...
  if (obs > 20) {
    arrows(20.2, 8.2, 20.2, 1, length=0.20, angle=15, lwd=3, col=1)
    text(x=18.8, y=10, 'OBSERVED', pos=1, cex=0.7, font=2)
  } else {
    arrows(obs, 8.2, obs, 1, length=0.20, angle=15, lwd=3, col=1)
    text(x=obs, y=10, 'OBSERVED', pos=1, cex=0.7, font=2)
  }
}
...
```{r plot_valuebar_S, fig.height=2.25, fig.width=6.0, fig.cap='Observed S deposition from CMAQ model (arrow) for the most recent 10-year sample period, in relation to levels of concern.  Levels of concern (Acceptable, Marginal, Concerning, Poor, Very poor) are defined by the expected decline of sensitive lichens (0, 0-20, 20-50, 50-80, and >80 percent declines established for critical loads).'}
...
We used critical loads from Table 3 in Geiser et al. (2019).  These critical loads are invariant to geography and so apply nationwide. Critical loads in this table are deposition levels resulting in $\geq20\%$ decrease in the richness or abundance. 
