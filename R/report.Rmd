...
if(len < 2) {
     ...
     t1 <- tryCatch(t.test(fmla_n,data=x),error=function(e)return(NA))
     t3 <- tryCatch(t.test(fmla_s,data=x),error=function(e)return(NA))
     ...
     tab1 <- tryCatch(do.call(data.frame, aggregate(fmla_n, data=x, f)),
                      error=function(e)return(NA))
     tab3 <- tryCatch(do.call(data.frame, aggregate(fmla_s, data=x, f)),
                      error=function(e)return(NA))
     has_ttest <- TRUE
}
```
...
rm(plot_ci, len, x, u, f, t1, t3, tab1, tab3, ylb_n, ylb_s, fmla_n, fmla_s)
```
