...
```{r plot-plot_valuebar_N, fig.height=2.5, fig.width=6.0, fig.cap='Observed N deposition from CMAQ model (arrow) for the most recent 10-year sample period, in relation to levels of concern.  Levels of concern (Acceptable, Marginal, Concerning, Poor, Very poor) are defined by the expected decline of sensitive lichens (0, 0-20, 20-50, 50-80, and >80 percent declines established for critical loads).'}
...
`plot_valuebar` <- function(pick, brk=b, select='n', ...){
    ...
    obs <-  x[length(x)]
  par(mar=c(4,.3,.3,.3), las=1, tcl=-0.2, mgp=c(1.8,0.4,0))
  n    <- 100
...
```{r plot_valuebar_S, fig.height=2.5, fig.width=6.0, fig.cap='Observed S deposition from CMAQ model (arrow) for the most recent 10-year sample period, in relation to levels of concern.  Levels of concern (Acceptable, Marginal, Concerning, Poor, Very poor) are defined by the expected decline of sensitive lichens (0, 0-20, 20-50, 50-80, and >80 percent declines established for critical loads).'}
...
```{r plot-map, fig.height=6.5, fig.width=6.49, fig.cap='Local area map.', warning=FALSE}
...
List of all macrolichens observed in plots at `r input$x` over the years `r rng[1]` to `r rng[2]`.  True diversity is likely greater because plot footprints are just a fraction of total Wilderness area.  

Functional groups ('Fxl grp') include cyanolichens ('cyano') capable of nitrogen-fixation, and forage lichens ('forage') important for wildlife.  'N sensitivity' for each species ranges from low to high tolerance (oligo-, meso-, eutroph), as does 'S sensitivity' (sensitive, intermediate, tolerant).  Critical loads ('CL') are deposition levels resulting in 20\% decrease in occurrence.  

'Conservation' = conservation status from NatureServe (https://www.natureserve.org/), which assigns each species both a global (G) and state (S) rank on a scale of 1 to 5 (1=Critically imperiled; 2=Imperiled; 3=Vulnerable; 4=Apparently secure; 5=Secure).  
