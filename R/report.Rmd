...
if(len < 2) {
     ...
     x <- x[x$tenyr %in% u,,drop=T]
     x <- x[order(x$tenyr), ]
     ### UNPAIRED 2-sample ttest - change in airscores (NOT paired...)
     t1 <- t.test(n_airscore ~ tenyr, data=x)
     t3 <- t.test(s_airscore ~ tenyr, data=x)
     ...
     tab1<-do.call(data.frame,aggregate(x$n_airscore,list(x$tenyr),f))
     tab3<-do.call(data.frame,aggregate(x$s_airscore,list(x$tenyr),f))
     has_ttest <- TRUE
}
```

`r if(has_ttest) {paste0('## Significant trends\n\nFrom the decade ', u[1], ' to ', u[2], ', a two-sample _t_-test of lichen airscores revealed ', ifelse(t1$p.value < 0.05,'a ','NO '), 'SIGNIFICANT trend in N deposition, and ', ifelse(t3$p.value < 0.05,'a ','NO '), 'SIGNIFICANT trend in S deposition. ', 'See figure for test results.')}`
...
set_par(2)
par(mgp=c(1.3,0.4,0))
plot_ci(tab1, t1, ylab=expression(N~air~score~(kg~N~ha^'-1'~y^'-1')))
plot_ci(tab3, t3, ylab=expression(S~air~score~(kg~S~ha^'-1'~y^'-1')))
rm(plot_ci, len, x, u, f, t1, t3, tab1, tab3)
```
...
__*Matrix lichens*__ are any that are not forage or cyanolichens.  They are often more pollution-tolerant than those two groups, and perform an array of ecosystem functions.  These functions include rainfall interception, water cycle buffering, animal habitat, and bioindication of forest health.
